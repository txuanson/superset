...
console.log('do we at least load the bundle???')

const LazyApp = lazy(
  () =>
    import(
      /* webpackChunkName: "QueryList" */ 'src/views/App'
      ),
);
...
try {
  ...
  if (!ALLOW_ORIGINS.includes(parentOrigin)) {
    throw new Error(`[superset] iframe parent ${parentOrigin} is not in the list of allowed origins`);
  }
  ...
  function validateMessageEvent(event: MessageEvent<any>) {
    if (event.data?.type === 'webpackClose' || event.data?.source === "@devtools-page") {
